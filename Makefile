...
OCAMLLIBS?=-I "theories"\
  -I "test-suite"\
  -I "src"
COQLIBS?=-Q "theories" HaskellExtraction\
  -I "theories"\
  -I "test-suite"\
  -I "src"
COQDOCLIBS?=-Q "theories" HaskellExtraction
...
CAMLC=ocamlfind ocamlc -c -rectypes -thread -linkpkg
...
VFILES:=test-suite/Test.v\
  theories/HaskellExtraction.v
...
VO=vo
VOFILES:=$(VFILES:.v=.$(VO))
VOFILESINC=$(filter $(wildcard theories/*),$(VOFILES)) $(filter $(wildcard test-suite/*),$(VOFILES)) 
...
NATIVEFILESINC=$(filter $(wildcard theories/*),$(NATIVEFILES)) $(filter $(wildcard test-suite/*),$(NATIVEFILES)) 
NATIVEFILES1=$(patsubst theories/%,%,$(filter theories/%,$(NATIVEFILES)))
ML4FILES:=src/haskell_extraction.ml4
...
MLLIBFILES:=src/haskell_extraction_plugin.mllib
...
install-natdynlink:
	install -d "$(DSTROOT)"$(COQLIBINSTALL)/HaskellExtraction; \
	for i in $(CMXSFILESINC); do \
	 install -m 0755 $$i "$(DSTROOT)"$(COQLIBINSTALL)/HaskellExtraction/`basename $$i`; \
	done
...
install:$(if $(HASNATDYNLINK_OR_EMPTY),install-natdynlink)
	cd "theories" && for i in $(NATIVEFILES1) $(GLOBFILES1) $(VFILES1) $(VOFILES1); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/HaskellExtraction/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/HaskellExtraction/$$i; \
	done
	for i in $(CMAFILESINC) $(CMIFILESINC) $(CMOFILESINC) $(NATIVEFILESINC) $(GLOBFILESINC) $(VFILESINC) $(VOFILESINC); do \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/HaskellExtraction/`basename $$i`; \
	done

install-doc:
	install -d "$(DSTROOT)"$(COQDOCINSTALL)/HaskellExtraction/html
	for i in html/*; do \
	 install -m 0644 $$i "$(DSTROOT)"$(COQDOCINSTALL)/HaskellExtraction/$$i;\
	done

uninstall_me.sh: Makefile
	echo '#!/bin/sh' > $@
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/HaskellExtraction && \\\nfor i in $(CMXSFILESINC); do rm -f "`basename "$$i"`"; done && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "HaskellExtraction" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/HaskellExtraction && rm -f $(NATIVEFILES1) $(GLOBFILES1) $(VFILES1) $(VOFILES1) && \\\nfor i in $(CMAFILESINC) $(CMIFILESINC) $(CMOFILESINC) $(NATIVEFILESINC) $(GLOBFILESINC) $(VFILESINC) $(VOFILESINC); do rm -f "`basename "$$i"`"; done && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "HaskellExtraction" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQDOCINSTALL)/HaskellExtraction \\\n' >> "$@"
	printf '&& rm -f $(shell find "html" -maxdepth 1 -and -type f -print)\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQDOCINSTALL) && find HaskellExtraction/html -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	chmod +x $@
