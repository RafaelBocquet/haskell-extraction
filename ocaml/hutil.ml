...
(* pretty *)
let pp_par p a = if p then Pp.str "(" ++ a ++ str ")" else a
...
let rec signature_view env c =
  ...
  | Prod (x, t, c) ->
    let args, ty = signature_view (push_rel_assum (x,t) env) c in
    (env, t) :: args, ty
  | _ -> [], (env, c)
...
let rec abstraction_view env c =
  match kind_of_term (whd_all env Evd.empty c) with
  | Lambda (x, t, c) ->
    let args, ty = abstraction_view (push_rel_assum (x,t) env) c in
    (env, t) :: args, ty
  | _ -> [], (env, c)
