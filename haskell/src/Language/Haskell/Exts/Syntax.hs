...
module Language.Haskell.Exts.Syntax (
    ...
    Context(..), FunDep(..), Asst(..),
    -- * Types
    Type(..), Kind, Boxed(..), TyVarBind(..), Promoted(..),
    TypeEqn (..),
    -- * Expressions
...
instance Annotated Type where
    ...
    amap f t1 = case t1 of
      ...
      KindStar l -> KindStar (f l)
      KindFn   l k1 k2 -> KindFn (f l) k1 k2
      KindParen l k -> KindParen (f l) k
      KindVar l n -> KindVar (f l) n
      KindApp l k1 k2 -> KindApp (f l) k1 k2
      KindTuple l ks -> KindTuple (f l) ks
      KindList  l ks -> KindList  (f l) ks
